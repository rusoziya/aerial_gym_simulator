...
class TestExponentialRewardFunction:
    ...
    def test_zero_value_gives_full_magnitude(self):
        assert torch.allclose(
            self.fn(2.0, 1.0, torch.tensor([0.0])), torch.tensor([2.0]), atol=1e-6
        )
    ...
    def test_symmetry(self):
        assert torch.allclose(
            self.fn(1.0, 1.0, torch.tensor([3.0])),
            self.fn(1.0, 1.0, torch.tensor([-3.0])),
            atol=1e-6,
        )
    ...
    def test_wider_base_width(self):
        assert (
            self.fn(1.0, 5.0, torch.tensor([1.0])).item()
            > self.fn(1.0, 0.5, torch.tensor([1.0])).item()
        )
...
class TestExponentialPenaltyFunction:
    ...
    def test_zero_gives_zero(self):
        assert torch.allclose(
            self.fn(1.0, 1.0, torch.tensor([0.0])), torch.tensor([0.0]), atol=1e-6
        )
    ...
    def test_larger_more_negative(self):
        assert (
            self.fn(1.0, 1.0, torch.tensor([3.0])).item()
            < self.fn(1.0, 1.0, torch.tensor([1.0])).item()
        )
        ...
        q = torch.tensor(
            [
                [
                    0.0,
                    0.0,
                    torch.sin(torch.tensor(a / 2)).item(),
                    torch.cos(torch.tensor(a / 2)).item(),
                ]
            ]
        )
