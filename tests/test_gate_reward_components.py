...
def _call(rp, N=1, pos_err=None, prev_err=None, crashes=None, action=None, prev_action=None,
          robot_pos=None, robot_orient=None, gate_pos=None, gate_passed=None, frac=0.0,
          gw=2.5, gh=2.4, gc=1.2, bv=False):
    ...
    return compute_gate_reward(pe, ppe, cr, a, pa, rp_, ro, gp, gpassed, frac, rp,
                                torch.full((N,), gw), torch.full((N,), gh), torch.full((N,), gc), bvm)


class TestGettingCloserReward:
    def test_getting_closer_positive(self, rp):
        r1, _, _ = _call(rp, pos_err=torch.tensor([[1.0,0,0]]), prev_err=torch.tensor([[2.0,0,0]]))
        r2, _, _ = _call(rp, pos_err=torch.tensor([[2.0,0,0]]), prev_err=torch.tensor([[2.0,0,0]]))
        assert r1[0].item() > r2[0].item()

    def test_getting_farther_penalized_more(self, rp):
        closer = _call(rp, pos_err=torch.tensor([[1.0,0,0]]), prev_err=torch.tensor([[2.0,0,0]]))[0]
        farther = _call(rp, pos_err=torch.tensor([[2.0,0,0]]), prev_err=torch.tensor([[1.0,0,0]]))[0]
        gain = closer[0].item() - _call(rp, pos_err=torch.tensor([[1.5,0,0]]), prev_err=torch.tensor([[1.5,0,0]]))[0][0].item()
        loss = _call(rp, pos_err=torch.tensor([[1.5,0,0]]), prev_err=torch.tensor([[1.5,0,0]]))[0][0].item() - farther[0].item()
        assert loss > gain  # 2x asymmetry


class TestGatePassageDetection:
    def test_past_gate_not_passed_triggers(self, rp):
        r, _, _ = _call(rp, robot_pos=torch.tensor([[0.0, 0.1, 1.2]]),
                         pos_err=torch.tensor([[0.01,0,0]]), prev_err=torch.tensor([[0.5,0,0]]))
        r_far, _, _ = _call(rp, robot_pos=torch.tensor([[0.0, -2.0, 1.5]]),
                             pos_err=torch.tensor([[2.0,0,0]]), prev_err=torch.tensor([[2.0,0,0]]))
        assert r[0].item() > r_far[0].item()

    def test_outside_width_no_passage_bonus(self, rp):
        r_center, _, _ = _call(rp, robot_pos=torch.tensor([[0.0, 0.1, 1.2]]),
                                pos_err=torch.tensor([[0.01,0,0]]), prev_err=torch.tensor([[0.5,0,0]]))
        r_wide, _, _ = _call(rp, robot_pos=torch.tensor([[5.0, 0.1, 1.2]]),
                              pos_err=torch.tensor([[5.01,0,0]]), prev_err=torch.tensor([[5.5,0,0]]))
...
class TestCameraAlignment:
    def test_facing_gate_alignment_positive(self, rp):
        _, _, align = _call(rp, robot_pos=torch.tensor([[0, -2, 1.5]]),
                            robot_orient=torch.tensor([[0, 0, 0, 1.0]]))
...
class TestActionPenalties:
    ...
    def test_action_diff_penalty(self, rp):
        r_same, _, _ = _call(rp, action=torch.tensor([[0.5,0.5,0.5,0.5]]),
                              prev_action=torch.tensor([[0.5,0.5,0.5,0.5]]))
        r_diff, _, _ = _call(rp, action=torch.tensor([[0.5,0.5,0.5,0.5]]),
                              prev_action=torch.tensor([[-0.5,-0.5,-0.5,-0.5]]))
