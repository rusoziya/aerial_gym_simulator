...
def _gate_reward(rp, N=1, **overrides):
    defaults = dict(
        pos_error=torch.ones(N, 3), prev_pos_error=torch.ones(N, 3),
        crashes=torch.zeros(N, dtype=torch.bool), action=torch.zeros(N, 4),
        prev_action=torch.zeros(N, 4), robot_pos=torch.tensor([[0,-1.5,1.5]]*N),
        robot_orient=torch.tensor([[0,0,0,1.0]]*N), gate_pos=torch.zeros(N, 3),
        gate_passed=torch.zeros(N, dtype=torch.bool), frac=0.0,
        gw=torch.full((N,), 2.5), gh=torch.full((N,), 2.4),
        gc=torch.full((N,), 1.2), bv=torch.zeros(N, dtype=torch.bool),
    )
    defaults.update(overrides)
    d = defaults
    return compute_gate_reward(d["pos_error"], d["prev_pos_error"], d["crashes"],
        d["action"], d["prev_action"], d["robot_pos"], d["robot_orient"],
        d["gate_pos"], d["gate_passed"], d["frac"], rp,
        d["gw"], d["gh"], d["gc"], d["bv"])
...
class TestGatePassedFlag:
    def test_already_passed_no_double_reward(self, rp):
        r_fresh, _, _ = _gate_reward(rp, gate_passed=torch.zeros(1, dtype=torch.bool),
                                      robot_pos=torch.tensor([[0.0, 0.1, 1.2]]),
                                      pos_error=torch.tensor([[0.01, 0, 0]]),
                                      prev_pos_error=torch.tensor([[0.5, 0, 0]]))
        r_passed, _, _ = _gate_reward(rp, gate_passed=torch.ones(1, dtype=torch.bool),
                                       robot_pos=torch.tensor([[0.0, 0.1, 1.2]]),
                                       pos_error=torch.tensor([[0.01, 0, 0]]),
                                       prev_pos_error=torch.tensor([[0.5, 0, 0]]))
