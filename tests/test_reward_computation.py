...
class TestComputeGateReward:
    ...
    def test_returns_three_tensors(self, zero_state, reward_params, num_envs):
        result = self.compute(
            zero_state["pos_error"], zero_state["prev_pos_error"],
            zero_state["crashes"], zero_state["action"], zero_state["prev_action"],
            zero_state["robot_position"], zero_state["robot_orientation"],
            zero_state["gate_position"], zero_state["gate_passed"],
            0.0, reward_params,
            zero_state["gate_width"], zero_state["gate_height"],
            zero_state["gate_center_height"], zero_state["boundary_violation"],
        )
        assert len(result) == 3
    ...
    def test_closer_to_target_gives_higher_reward(self, zero_state, reward_params, num_envs):
        ...
        args = lambda s: (s["pos_error"], s["prev_pos_error"], s["crashes"],
                          s["action"], s["prev_action"], s["robot_position"],
                          s["robot_orientation"], s["gate_position"], s["gate_passed"],
                          0.0, reward_params, s["gate_width"], s["gate_height"],
                          s["gate_center_height"], s["boundary_violation"])
    ...
    def test_curriculum_fraction_scales_reward(self, zero_state, reward_params, num_envs):
        args = lambda frac: (zero_state["pos_error"], zero_state["prev_pos_error"],
                             zero_state["crashes"], zero_state["action"], zero_state["prev_action"],
                             zero_state["robot_position"], zero_state["robot_orientation"],
                             zero_state["gate_position"], zero_state["gate_passed"],
                             frac, reward_params, zero_state["gate_width"],
                             zero_state["gate_height"], zero_state["gate_center_height"],
