...
class TestRewardParameters:
    ...
    def test_pos_reward_exponent(self):
        assert self.rp["pos_reward_exponent"] == 0.15

    def test_very_close_magnitude(self):
        assert self.rp["very_close_to_goal_reward_magnitude"] == 0.75
    ...
    def test_gate_passage(self):
        assert self.rp["gate_passage_reward_magnitude"] == 100.0

    def test_gate_center_passage(self):
        assert self.rp["gate_center_passage_bonus_magnitude"] == 100.0

    def test_gate_approach(self):
        assert self.rp["gate_approach_reward_magnitude"] == 1.25

    def test_gate_alignment(self):
        assert self.rp["gate_alignment_reward_magnitude"] == 0.5

    def test_gate_center_bonus(self):
        assert self.rp["gate_center_bonus_magnitude"] == 1.25
    ...
    def test_time_penalty_total(self):
        assert self.rp["time_penalty_total_at_horizon"] == 4.0

    def test_camera_facing_disabled(self):
        assert self.rp["camera_facing_reward_magnitude"] == 0.0

    def test_image_penalty_magnitude(self):
        assert self.rp["image_penalty_magnitude"] == 0.2
...
class TestCurriculumSchedules:
    ...
    def test_obstacles_at_min(self):
        assert self.cur.get_obstacle_count_behind_gate(3) == 3

    def test_obstacles_at_max(self):
        assert self.cur.get_obstacle_count_behind_gate(23) == 10
    ...
    def test_spawn_widens(self):
        assert (
            self.cur.get_spawn_ranges(23)["x_half_span_m"]
            > self.cur.get_spawn_ranges(3)["x_half_span_m"]
        )
