...
def compute_inflation_kernel(
    image_w, image_h, hfov_deg, vfov_deg, robot_w_m, robot_h_m, ref_dist_m
) -> None:
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="Visualize collision-image pipeline (depth → noise/dropout → edges → inflated depth → collision image)"
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=6,
        help="Number of depth bins for variable inflation (closer to paper)",
    )
    p.add_argument(
        "--dilate3d",
        action="store_true",
        help="Enable full 3D dilation in camera space (uvz voxel). Slower but closer to paper",
    )
    p.add_argument(
    ...
    kh, kw = compute_inflation_kernel(
        args.image_w,
        args.image_h,
        args.hfov_deg,
        args.vfov_deg,
