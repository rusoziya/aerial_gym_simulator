...
from verifiable_learning.DepthToLatent.datasets.depth_collision_image_dataset import (
    collate_batch,
)
from verifiable_learning.DepthToLatent.datasets.depth_semantic_image_dataset import (
    DepthSemanticImageDataset,
    collate_batch,
)
...
class CollisionImageProcessor:
    def __init__(self, cam_params, edge_detector, image_processor):
        ...
        self.meshgrid = create_meshgrid(
            270, 480, cam_params.cx, cam_params.cy, cam_params.fx, cam_params.fy
        )
    ...
    def process_image(self, depth_img) -> None:
        ...
        point_cloud, offset_image = depth_to_pointcloud(
            depth_img, self.meshgrid, scale=1.0, offset_dist=0.2
        )
        ...
        faces = wp.array(
            np.array(cube_mesh_aggregated.faces.flatten()), dtype=wp.int32, device="cuda:0"
        )
        ...
        return (
            combined_collision_image,
            normalized_raycast_image,
            normalized_offset_image,
            edge_image,
        )
...
def main() -> None:
    ...
    test_dataset = DepthSemanticImageDataset(
        tfrecord_folder=TFRECORD_FOLDER, shuffle=True, batch_size=1, one_tfrecord=False
    )
    ...
    counter = 0
    for batch_idx, (num_envs, images_per_env, depth_data, filtered_data, semantic_data) in tqdm(
        ...
        processed_image, *_ = collision_image_processor.process_image(
        ...
        processed_collision_image_full_depth_uint16 = (
            1000.0 * processed_collision_image_full_depth
        ).astype(np.uint16)
        depth_data_torch_true_depth_uint16 = (
            1000.0 * depth_data_torch_true_depth[0, 0].numpy()
        ).astype(np.uint16)

        out_pickle_list.append(
            [processed_collision_image_full_depth_uint16, depth_data_torch_true_depth_uint16]
        )
