...
class task_config:
    ...
    controller_name = (
        "lmf2_velocity_control"  # CHANGED: Switch to velocity controller for more direct control
    )
    args = {}
    num_envs = 16  # Default to 16 environments for gate navigation training
    use_warp = True
    headless = (
        False  # Enable visualization to view both cameras (can be overridden by Sample Factory)
    )
    device = "cuda:0"
    ...
    observation_space_dim = (
        150  # Enhanced to include drone position and full orientation = 150D (3+6+3+10+64+64)
    )
    ...
    class curriculum:
        ...
        enable_dynamic_camera_following = (
            False  # Toggle: False = static camera, True = follow drone
        )
        ...
        dynamic_camera_follow_distance_z = (
            0.0  # No Z offset as requested (camera at same height as drone)
        )
        ...
        def update_curriculim_level(self, success_rate, current_level):
            from aerial_gym.config.task_config.curriculum_schedules import update_curriculim_level

            return update_curriculim_level(success_rate, current_level)

        @staticmethod
        def get_obstacle_count_behind_gate(level):
            from aerial_gym.config.task_config.curriculum_schedules import (
                get_obstacle_count_behind_gate,
            )

            return get_obstacle_count_behind_gate(level)

        @staticmethod
        def get_camera_noise(level):
            from aerial_gym.config.task_config.curriculum_schedules import get_camera_noise

            return get_camera_noise(level)

        @staticmethod
        def get_camera_frame_dropout(level):
            from aerial_gym.config.task_config.curriculum_schedules import get_camera_frame_dropout

            return get_camera_frame_dropout(level)

        @staticmethod
        def get_state_noise(level):
            from aerial_gym.config.task_config.curriculum_schedules import get_state_noise

            return get_state_noise(level)

        @staticmethod
        def get_spawn_ranges(level):
            from aerial_gym.config.task_config.curriculum_schedules import get_spawn_ranges

            return get_spawn_ranges(level)

        @staticmethod
        def get_static_camera_difficulty(level):
            from aerial_gym.config.task_config.curriculum_schedules import (
                get_static_camera_difficulty,
            )

            return get_static_camera_difficulty(level)

        @staticmethod
        def get_dynamic_camera_follow_offset():
            from aerial_gym.config.task_config.curriculum_schedules import (
                get_dynamic_camera_follow_offset,
            )

            return get_dynamic_camera_follow_offset()
    ...
    class static_camera_curriculum:
        ...
        def get_camera_position_and_orientation(level, gate_position, env_bounds) -> None:
            ...
            camera_angle, height_offset, distance_offset = (
                task_config.curriculum.get_static_camera_difficulty(level)
            )
