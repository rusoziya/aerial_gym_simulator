...
from aerial_gym.config.asset_config.gate_asset_config import (
    gate_asset_params,
    small_gate_asset_params,
)
from aerial_gym.config.env_config.gate_env_walls import (
    boundary_back_wall,
    boundary_bottom_wall,
    boundary_front_wall,
    boundary_left_wall,
    boundary_right_wall,
    boundary_top_wall,
...
class GateEnvCfg:
    ...
    class env_config:
        ...
        _DISABLE_WALLS = (os.environ.get("DISABLE_WALLS", "false").lower() == "true") or (
            os.environ.get("SF_DISABLE_WALLS", "false").lower() == "true"
        )
        ...
        class gate_object_params(BaseAssetParams):
            ...
            min_state_ratio = [
                0.1,
                0.65,
                0.0,  # X: wider spread from left, Y: behind gate (positive Y), Z: on ground
                0,
                -np.pi / 3.0,
                -np.pi,  # More rotation variation
            ...
            max_state_ratio = [
                0.9,
                0.95,
                0.55,  # X: wider spread to right, Y: well behind gate, Z: gate top (2.2m)
                0,
                np.pi / 3.0,
                np.pi,  # More rotation variation
