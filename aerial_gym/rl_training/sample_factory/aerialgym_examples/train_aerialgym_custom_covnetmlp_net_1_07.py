        ...
        try:
            ...
            try:
                ...
                if hasattr(cfg, "env_agents") and cfg.env_agents > 0:
                    ...
                    print(
                        f"[SUBPROCESS] Setting num_envs to {cfg.env_agents} based on env_agents={cfg.env_agents}"
                    )
                    print(f"[SUBPROCESS] Set SF_ENV_AGENTS={cfg.env_agents} environment variable")
                    print(
                        f"[SUBPROCESS] DCE config batch_size: {getattr(cfg, 'batch_size', 'not set')}"
                    )
                    if cfg.env_agents == 1:
                        print("[SUBPROCESS] Using single environment (low memory) configuration")
                    elif cfg.env_agents == 4:
                        print("[SUBPROCESS] Using medium configuration (4 environments)")
                    elif cfg.env_agents == 6:
                        print("[SUBPROCESS] Using OPTIMIZED configuration (6 environments)")
                    elif cfg.env_agents == 8:
                        print("[SUBPROCESS] Using high performance configuration (8 environments)")
                    elif cfg.env_agents == 16:
                        print("[SUBPROCESS] Using original DCE configuration (16 environments)")
                    else:
                        print(
                            f"[SUBPROCESS] Using custom {cfg.env_agents} environment configuration"
                        )
                else:
                    print(
                        f"[SUBPROCESS] env_agents={getattr(cfg, 'env_agents', 'not set')}, using default num_envs"
                    )
                ...
                print("Registered quad_with_obstacles and dce_navigation_task in subprocess")
...
def register_aerialgym_custom_components():
    ...
    if current_env_agents == "6":
        print(
            f"Set SF_ENV_AGENTS={current_env_agents} environment variable for all processes (OPTIMIZED config)"
        )
    elif current_env_agents == "4":
        print(
            f"Set SF_ENV_AGENTS={current_env_agents} environment variable for all processes (MEDIUM config)"
        )
    else:
        print(
            f"Set SF_ENV_AGENTS={current_env_agents} environment variable for all processes (CUSTOM config)"
        )
