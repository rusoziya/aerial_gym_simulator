    ...
    def __init__(self, layer_sizes, lims):
        ...
        if len(layer_sizes) > 2:
            ...
            for i, input_size in enumerate(layer_sizes[1:-1]):
                ...
                self.control_stack.append(nn.Linear(input_size, output_size).to(torch.float)).cpu()
                self.control_stack.append(nn.Tanh())

    def rescale_actions(self, scaled_command_actions) -> None:
        command_actions = scaled_command_actions.clone()
        command_actions = (
            scaled_command_actions * (self.max_u - self.min_u) / 2 + (self.max_u + self.min_u) / 2
        )
...
def convert_model_to_script_model(nn_model_full, max_u, min_u, n_motors) -> None:
    ...
    nn_model_deploy.control_stack[0].weight.data[:] = (
        nn_model_full.actor_critic.actor_encoder.encoders.observations.mlp_head[0].weight.data
    )
    nn_model_deploy.control_stack[0].bias.data[:] = (
        nn_model_full.actor_critic.actor_encoder.encoders.observations.mlp_head[0].bias.data
    )
    nn_model_deploy.control_stack[2].weight.data[:] = (
        nn_model_full.actor_critic.actor_encoder.encoders.observations.mlp_head[2].weight.data
    )
    nn_model_deploy.control_stack[2].bias.data[:] = (
        nn_model_full.actor_critic.actor_encoder.encoders.observations.mlp_head[2].bias.data
    )
    nn_model_deploy.control_stack[4].weight.data[:] = (
        nn_model_full.actor_critic.action_parameterization.distribution_linear.weight.data
    )
    nn_model_deploy.control_stack[4].bias.data[:] = (
        nn_model_full.actor_critic.action_parameterization.distribution_linear.bias.data
    )
