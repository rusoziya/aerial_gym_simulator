...
from aerial_gym.rl_training.sample_factory.end_to_end_training.deployment.convert_model import (
    convert_model_to_script_model,
)
...
def test_policy_script_export() -> None:
    ...
    env = task_registry.make_task(
        "position_setpoint_task_sim2real_end_to_end", num_envs=1, headless=True
    )
    obs = env.reset()[0]
    ...
    for i in range(n_steps):
        ...
        actions_motor_commands = (
            model_deploy(obs["observations"].squeeze().cpu()).unsqueeze(0).to("cuda")
        )
        ...
        pos_err_list.append(
            (goals[reset_counter - 1].cpu() + obs["observations"].squeeze().cpu()[0:3])
            .detach()
            .numpy()
            .tolist()
        )
    ...
    plot_results(
        ...
        ang_vel_list,
        action_motor_command_list,
        goals.cpu().numpy().tolist(),
        dt,
    )


def plot_results(
    pos_list, pos_err_list, vel_list, ori_list, ang_vel_list, action_motor_command_list, goals, dt
) -> None:
    ...
    ax.scatter(
        np.array(goals)[:, 0], np.array(goals)[:, 1], np.array(goals)[:, 2], c="r", marker="o"
    )
    plt.show()
