...
class MultiAgentNavigationTask(MultiAgentBaseTask):
    ...
    def __init__(self, task_config):
        ...
        self.target_positions_config = getattr(
            task_config,
            "target_positions",
            [
        ...
        self.init_positions_config = getattr(
            task_config,
            "init_positions",
            [
        ...
        self.target_positions = (
            torch.tensor(self.target_positions_config, device=self.device, dtype=torch.float32)
            .unsqueeze(0)
            .repeat(self.num_envs, 1, 1)
        )  # (num_envs, num_robots, 3)

        self.init_positions = (
            torch.tensor(self.init_positions_config, device=self.device, dtype=torch.float32)
            .unsqueeze(0)
            .repeat(self.num_envs, 1, 1)
        )  # (num_envs, num_robots, 3)
        ...
        print("✅ MultiAgentNavigationTask initialized:")
    ...
    def _setup_multi_agent_spaces(self):
        ...
        self.multi_agent_observation_space = gym.spaces.Box(
            low=-np.inf, high=np.inf, shape=(self.num_robots_per_env, obs_dim), dtype=np.float32
        )
        ...
        self.multi_agent_action_space = gym.spaces.Box(
            low=-1.0, high=1.0, shape=(self.num_robots_per_env, action_dim), dtype=np.float32
        )
        ...
        if hasattr(self.sim_env, "robot_manager") and hasattr(self.sim_env.robot_manager, "robot"):
            for env_idx in env_ids:
                for robot_idx in range(self.num_robots_per_env):
                    ...
                    self.sim_env.robot_manager.robot.robot_state[abs_robot_idx, 7:10] = (
                        0.0  # linear velocity
                    )
                    self.sim_env.robot_manager.robot.robot_state[abs_robot_idx, 10:13] = (
    ...
    def get_multi_agent_observations(self):
        ...
        all_robot_states = obs_dict.get(
            "robot_state_tensor",
            obs_dict.get(
                "robot_state",
                torch.zeros(self.num_envs * self.num_robots_per_env, 13, device=self.device),
            ),
        )
            ...
            for robot_idx in range(self.num_robots_per_env):
                ...
                for other_robot_idx in range(self.num_robots_per_env):
                    if other_robot_idx != robot_idx and other_robot_count < 2:
                        ...
                        processed_obs[env_idx, robot_idx, obs_idx : obs_idx + 3] = (
                            relative_other_pos
                        )
                        obs_idx += 3
                        other_robot_count += 1
    ...
    def _check_multi_agent_termination(self):
        ...
        for env_idx in range(self.num_envs):
            ...
            if self.current_episode_steps[env_idx] >= self.episode_len_steps:
                print(
                    f"   DEBUG: Environment {env_idx} terminated due to episode length: {self.current_episode_steps[env_idx]} >= {self.episode_len_steps}"
                )
                env_done = True
            else:
                ...
                for robot_idx in range(self.num_robots_per_env):
                    ...
                    out_of_bounds = (
                        ...
                        | (robot_pos[1] > 50)
                        | (robot_pos[2] < -5)
                        | (robot_pos[2] > 20)  # Very permissive height bounds for debugging
                    )

                    if out_of_bounds:
                        print(
                            f"   DEBUG: Environment {env_idx}, Robot {robot_idx} out of bounds: pos={robot_pos}"
                        )
                        env_done = True
                        break
                    ...
                    for other_idx in range(self.num_robots_per_env):
                        if other_idx != robot_idx:
                            ...
                            if distance < self.hard_collision_radius:
                                print(
                                    f"   DEBUG: Environment {env_idx}, Robot {robot_idx} collision with robot {other_idx}: distance={distance:.3f} < {self.hard_collision_radius}"
                                )
                                env_done = True
                                break
    ...
    def _get_multi_agent_info(self):
        ...
        info = {"target_distances": [], "formation_error": None, "collision_count": 0}
