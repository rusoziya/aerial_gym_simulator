...
class MultiAgentBaseTask(BaseTask):
    ...
    def __init__(self, task_config: object) -> None:
        super().__init__(task_config)

        self.num_robots_per_env: int = task_config.num_robots_per_env
        self.enable_inter_agent_obs: bool = task_config.enable_inter_agent_obs
        self.communication_range: float = task_config.communication_range
        self.shared_reward: bool = task_config.shared_reward
        self.individual_rewards: bool = task_config.individual_rewards
        self.device: str = task_config.device
        self.num_envs: int = task_config.num_envs
        self.env_manager: object | None = None
        self.observation_space: object | None = None
        self.action_space: object | None = None
    ...
    def _setup_multi_agent_spaces(self) -> None:
        ...
        single_obs_dim = (
            self.observation_space.shape[0] if self.observation_space is not None else 10
        )
        single_action_dim = self.action_space.shape[0] if self.action_space is not None else 4
    ...
    def get_multi_agent_observations(self) -> torch.Tensor:
        """Get observations for all robots in all environments."""
        if self.env_manager is None:
            obs_dim = self.observation_space.shape[0] if self.observation_space is not None else 13
            return torch.zeros(self.num_envs, self.num_robots_per_env, obs_dim, device=self.device)

        obs_dict = self.env_manager.get_obs()
        robot_state = obs_dict.get(
            "robot_state", torch.zeros(self.num_envs, 13, device=self.device)
        )

        if self.num_robots_per_env == 1:
            return robot_state.unsqueeze(1)
        return robot_state.unsqueeze(1).expand(-1, self.num_robots_per_env, -1)
    ...
    def _get_inter_agent_observations(self, robot_idx) -> None:
        ...
        for other_robot_idx in range(self.num_robots_per_env):
            ...
            inter_obs = (
                inter_obs * comm_mask
            )  # Zero out observations for robots outside communication range
    ...
    def compute_formation_rewards(self, desired_formation, formation_scale=1.0) -> None:
        ...
        desired_positions = center_of_mass.unsqueeze(1) + scaled_formation.unsqueeze(
            0
        )  # (num_envs, num_robots, 3)

        # Calculate formation error
        position_errors = torch.norm(
            self.robot_positions - desired_positions, dim=-1
        )  # (num_envs, num_robots)
