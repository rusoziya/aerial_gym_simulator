...
def save_camera_images(rl_task, step_num) -> None:
    ...
    try:
        ...
        plt.tight_layout()
        plt.savefig(
            f"camera_outputs/dual_camera_step_{step_num:03d}.png", dpi=150, bbox_inches="tight"
        )
        plt.close()
...
def test_gate_navigation_dual_camera() -> None:
    ...
    task_registry.register_task("navigation_task_gate_test", DCE_RL_Navigation_Task_Gate, config)
    ...
    print("  Memory optimization: Single VAE for both cameras")
    print()
    ...
    for step in range(100):
        ...
        if step % 5 == 0:
            print(
                f"Step {step:3d}: obs_shape={obs['observations'].shape}, "
