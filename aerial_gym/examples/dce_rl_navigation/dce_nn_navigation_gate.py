...
from aerial_gym.examples.dce_rl_navigation.inference_utils import (
    to_pil_gray,
    save_gif,
    wilson_ci,
)
...
def _init_task_and_model(
    ...
    headless: bool,
) -> tuple[DCE_RL_Navigation_Task_Gate, NN_Inference_Class_Gate, int, int]:
    """Create the RL task and inference model.

    Returns (rl_task, nn_model, num_actions, num_obs).
    """
    ...
    if use_wandb:
        try:
            ...
            entity = (
                ...
                os.environ.get('WANDB_ENTITY', '') or
                cfg.wandb_user
            ) or None
...
def _print_eval_config_summary(
    rl_task: DCE_RL_Navigation_Task_Gate,
    nn_model: NN_Inference_Class_Gate,
    nn_obs_key: str,
) -> None:
    """Print a one-time evaluation config summary to stdout."""
    try:
        print("=" * 80)
        ...
        def _p(key: str, label: str | None = None) -> None:
            if key in g:
        ...
        print(f"  ABLATE_OBS_RANGES: {os.environ.get('ABLATE_OBS_RANGES', '')}")
        print("=" * 80)
    except (ValueError, TypeError):
        pass
...
def _run_rnn_warmup(
    ...
    Returns the (potentially updated) obs_dict after warm-up.
    """
    try:
...
def _run_inference_loop(
    ...
    # Print one-time eval config summary
    _print_eval_config_summary(rl_task, nn_model, nn_obs_key)
            ...
            try:
                ...
                if isinstance(d, dict) and 'depth_range_pixels_noised' in d:
                    ...
                    if isinstance(dc, torch.Tensor):
                        if dc.ndim == 4:  # (N,C,H,W)
                            pil_clean = to_pil_gray(dc[0, 0])
                        elif dc.ndim == 3:  # (N,H,W)
                            pil_clean = to_pil_gray(dc[0])
                        elif dc.ndim == 2:  # (H,W)
                            pil_clean = to_pil_gray(dc)
                    elif dc is not None:
                        pil_clean = to_pil_gray(dc)
                ...
                if isinstance(d, dict) and 'static_depth_noised' in d:
                    ...
                    if isinstance(sc, torch.Tensor):
                        if sc.ndim == 3:
                            pil_sclean = to_pil_gray(sc[0])
                        elif sc.ndim == 2:
                            pil_sclean = to_pil_gray(sc)
                    elif sc is not None:
                        pil_sclean = to_pil_gray(sc)
                    ...
                    if scm is not None and hasattr(scm, 'capture_images'):
                        ...
                        if _d is not None:
                            if isinstance(_d, np.ndarray):
                                if _d.ndim == 3:  # (N,H,W)
                                    pil_d = to_pil_gray(_d[0])
                                else:  # (H,W)
                                    pil_d = to_pil_gray(_d)
                            else:
                                pil_d = to_pil_gray(_d)
        ...
        try:
            ...
            if (terminated is not None and truncated is not None and
                ...
                try:
                    ...
                    if ids.numel() > 0:
                        ids_cpu = ids.cpu()
                        def _update_running(sum_key_env: str, count_key_env: str, sum_key_run: str, count_key_run: str) -> None:
                        ...
                        if wandb_run is not None:
                            ...
                            def _safe_run_mean(s_key: str, c_key: str) -> float:
                    ...
                    if save_gifs and (ids.numel() > 0) and (0 in ids.cpu().tolist()):
                        ...
                        if (_gif_episode_counter_env0 % _gif_every_n) == 0:
                            save_gif(_gif_drone_noised_frames, gif_dir, f"episode_{_gif_episode_counter_env0:04d}_drone_depth_D455_NOISED.gif")
                            save_gif(_gif_drone_clean_frames, gif_dir, f"episode_{_gif_episode_counter_env0:04d}_drone_depth_D455_CLEAN.gif")
                            save_gif(_gif_drone_seg_frames, gif_dir, f"episode_{_gif_episode_counter_env0:04d}_drone_seg.gif")
                            save_gif(_gif_static_noised_frames, gif_dir, f"episode_{_gif_episode_counter_env0:04d}_static_depth_D455_NOISED.gif")
                            save_gif(_gif_static_clean_frames, gif_dir, f"episode_{_gif_episode_counter_env0:04d}_static_depth_D455_CLEAN.gif")
                            save_gif(_gif_static_seg_frames, gif_dir, f"episode_{_gif_episode_counter_env0:04d}_static_seg.gif")
...
def main() -> None:
    ...
    with torch.no_grad():
        wandb_run, use_wandb = _init_wandb(cfg, args)
