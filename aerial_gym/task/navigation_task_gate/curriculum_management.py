    ...
    def check_and_update_curriculum_level(self, successes: torch.Tensor, crashes: torch.Tensor, timeouts: torch.Tensor) -> None:
        ...
        if forced is not None and str(forced).lower() != 'no':
            ...
            effective_max = (
                ...
                if eval_stretch_enabled else self.task.task_config.curriculum.max_level
            )
        ...
        instances = self.task.success_aggregate + self.task.crashes_aggregate + self.task.timeouts_aggregate
        ...
        if instances >= self.task.task_config.curriculum.check_after_log_instances:
            ...
            self.task.log_curriculum_update(f"[CURRICULUM UPDATE] EVALUATING curriculum after {instances} instances:")
            ...
            self.task.log_curriculum_update(f"[CURRICULUM UPDATE]   Current level: {old_level} (max reached: {self.task.max_curriculum_level_reached})")
            self.task.log_curriculum_update(f"[CURRICULUM UPDATE]   Thresholds: increase>{self.task.task_config.curriculum.success_rate_for_increase:.3f}, decrease<{self.task.task_config.curriculum.success_rate_for_decrease:.3f}")
            ...
            if self.task._curriculum_cooldown > 0:
                ...
                if (len(self.task._success_window_history) >= 3) and (s_t >= inc_threshold) and (avg3 >= avg3_threshold):
                ...
                elif success_rate < self.task.task_config.curriculum.success_rate_for_decrease and self.task.curriculum_level > self.task.task_config.curriculum.min_level:
            ...
            if forced is not None:
                ...
                self.task.success_aggregate = 0; self.task.crashes_aggregate = 0; self.task.timeouts_aggregate = 0
            ...
            try:
                obs_dis = bool(self.task.sim_env.global_tensor_dict.get('obstacles_randomization/disabled', False))
            except (KeyError, TypeError):
                obs_dis = False
            if obs_dis:
                try:
                    obstacles_behind_gate = int(self.task.sim_env.global_tensor_dict.get('obstacles_randomization/fixed_count', 0))
                except (ValueError, TypeError):
            ...
            self.task.log_curriculum_update(f"[CAMERA UPDATE] Camera max angle updated for NEW episodes: ±{self.task.max_camera_angle:.1f}° (existing episodes unchanged)")
            ...
            self.task._log_curriculum_details(success_rate, crash_rate, timeout_rate, obstacles_behind_gate, total_obstacles_in_env)

            self.task._populate_curriculum_infos(success_rate, crash_rate, timeout_rate, obstacles_behind_gate, total_obstacles_in_env)

            self.task.log_curriculum_update(f"[CURRICULUM UPDATE] RESETTING counters for next evaluation period")
