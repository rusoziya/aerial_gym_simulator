...
def _init_gate_tracking_tensors(task) -> None:
    """Initialise gate-specific tracking tensors and adaptive dimensions."""
    task.gate_position = torch.zeros((task.sim_env.num_envs, 3), device=task.device)
    task.gate_approach_distance = torch.zeros(task.sim_env.num_envs, device=task.device)
    task.gate_width = torch.zeros((task.sim_env.num_envs,), device=task.device)
    task.gate_height = torch.zeros((task.sim_env.num_envs,), device=task.device)
    task.gate_center_height = torch.zeros((task.sim_env.num_envs,), device=task.device)
    task.gate_scale_factors = torch.ones((task.sim_env.num_envs,), device=task.device)

def _init_vae_model(task) -> None:
    """Set up shared VAE encoder (or identity fallback) and latent buffers."""
    if task.task_config.vae_config.use_vae:
        task.shared_vae_model = VAEImageEncoder(
            config=task.task_config.vae_config, device=task.device
        )
        latent_dims = task.task_config.vae_config.latent_dims
        task.image_latents = torch.zeros(
            (task.sim_env.num_envs, latent_dims),
            device=task.device,
            requires_grad=False,
        )
        task.static_image_latents = torch.zeros(
            (task.sim_env.num_envs, latent_dims),
            device=task.device,
            requires_grad=False,
        )
    else:
        task.shared_vae_model = lambda x: x
        task.image_latents = torch.zeros(
            (task.sim_env.num_envs, 1), device=task.device, requires_grad=False
        )
        task.static_image_latents = torch.zeros(
            (task.sim_env.num_envs, 1), device=task.device, requires_grad=False
        )

def _init_observation_action_spaces(task) -> None:
    """Define Gymnasium observation and action spaces."""
    task.observation_space = Dict(
        {
            "observations": Box(
                low=-1.0,
                high=1.0,
                shape=(task.task_config.observation_space_dim,),
                dtype=np.float32,
            ),
            "image_obs": Box(
                low=-1.0,
    ...
    task.action_space = Box(low=-1.0, high=1.0, shape=(4,), dtype=np.float32)
    task.action_transformation_function = task.task_config.action_transformation_function
    task.num_envs = task.sim_env.num_envs

def _init_task_observations(task) -> None:
    """Allocate task observation tensors."""
    task.task_obs = {
        "observations": torch.zeros(
            (task.sim_env.num_envs, task.task_config.observation_space_dim),
            device=task.device,
            requires_grad=False,
        ),
        "priviliged_obs": torch.zeros(
            (
                task.sim_env.num_envs,
                task.task_config.privileged_observation_space_dim,
            ),
            device=task.device,
            requires_grad=False,
        ),
...
def _init_episode_reward_tracking(task) -> None:
    """Allocate per-component episode reward accumulators and statistics tensors."""
    task.episode_rewards = EpisodeRewardAccumulators.create(task.num_envs, task.device)
    # Backward-compatible aliases (used by update_episode_reward_tracking, reset, logging)
    task.episode_pos_reward = task.episode_rewards.pos_reward
    task.episode_very_close_reward = task.episode_rewards.very_close_reward
    task.episode_getting_closer_reward = task.episode_rewards.getting_closer_reward
    task.episode_gate_approach_reward = task.episode_rewards.gate_approach_reward
    task.episode_gate_alignment_reward = task.episode_rewards.gate_alignment_reward
    task.episode_camera_facing_reward = task.episode_rewards.camera_facing_reward
    task.episode_action_penalty = task.episode_rewards.action_penalty
    task.episode_gate_passage_reward = task.episode_rewards.gate_passage_reward
    task.episode_collision_penalty = task.episode_rewards.collision_penalty
    task.episode_image_reward = task.episode_rewards.image_reward
    task.episode_static_fov_visibility_reward = task.episode_rewards.static_fov_visibility_reward
    task.episode_boundary_violation_penalty = task.episode_rewards.boundary_violation_penalty
    task.episode_lengths = task.episode_rewards.lengths
    task.episode_time_penalty = task.episode_rewards.time_penalty
    task.episode_timeout_penalty = task.episode_rewards.timeout_penalty
    task.completed_episodes: list[dict[str, float]] = []
    task.max_stored_episodes = 10

def _init_episode_trajectory_state(task) -> None:
    """Allocate per-environment episode trajectory tracking state."""
    task.trajectory = EpisodeTrajectoryState.create(task.num_envs, task.device)
    # Backward-compatible aliases (used throughout step/reset/curriculum code)
    task._episode_fresh = task.trajectory.fresh
    task._ep_spawn_pos = task.trajectory.spawn_pos
    task._ep_gate_center_at_spawn = task.trajectory.gate_center_at_spawn
    task._ep_last_pos = task.trajectory.last_pos
    task._ep_path_len = task.trajectory.path_len
    task._ep_steps = task.trajectory.steps
    task._ep_min_gate_dist = task.trajectory.min_gate_dist
    task._ep_gate_crossed = task.trajectory.gate_crossed
    task._ep_time_to_gate = task.trajectory.time_to_gate
    task._ep_center_offset_cross = task.trajectory.center_offset_cross
    task._ep_height_offset_cross = task.trajectory.height_offset_cross
    task._bv_flag_episode = task.trajectory.bv_flag_episode
    task._bv_prev_env0 = False

def _init_debug_flags(task) -> None:
    """Initialize one-shot debug/logging flags so hasattr checks are never needed."""
    task._drone_cam_debug_last = False
    task._drone_vae_debug_last = False
    task._static_cam_debug_last = False
    task._static_cam_depth_logged = False
    task._static_debug_logged = False
    task._vae_output_logged = False
    task._vae_debug_logged = False
    task._no_static_logged = False
    task._return_tuple_debug_printed = False
    task._final_verification_printed = False
    task._prev_drone_depth = None
    task._prev_static_depth = None
    task._success_window_history = []
    task._curriculum_cooldown = 0
    task.min_pixel_dist = None
    task.curriculum_log_file = None
    task._curriculum_multiplier_factor = 1.0
    task._debug_cam_world = None
    task._debug_drone_eul = None
    task._debug_cam_eul = None
    task.max_curriculum_level_reached = task.curriculum_level
    task._last_traj_metrics_per_env = {}
    task._last_traj_metrics_avg = {}

def _init_curriculum(task) -> None:
    ...
    task.curriculum_log_file = None  # Initialize to None
    task.setup_curriculum_logging()  # Set up logging before using it
    ...
    logger.warning(f"[OBSTACLE_FIX] CURRICULUM: Level {task.curriculum_level} should spawn {obstacles_behind_gate} curriculum obstacles")
    ...
    if yaw_enabled and not dynamic_effective:
        ...
        logger.info(f"   4. CAMERA ANGLE: ±{task.max_camera_angle:.1f}deg max range (randomized per episode reset, fixed during episode)")
    
    # 5. CAMERA NOISE PROGRESSION (D455 Simulation)
    initial_camera_gaussian_std, initial_camera_dropout_rate = task.task_config.curriculum.get_camera_noise(task.curriculum_level)
    ...
    logger.info(f"   8. PROGRESS: {task.curriculum_progress_fraction:.3f} (level {task.curriculum_level}/{task.task_config.curriculum.max_level})")
    logger.info(f"   9. EVALUATION: Check every {task.task_config.curriculum.check_after_log_instances} instances (success rate threshold: {task.task_config.curriculum.success_rate_for_increase:.3f})")
    
    task.log_curriculum_update(f"[INIT] Multi-aspect curriculum initialized at level {task.curriculum_level}")
