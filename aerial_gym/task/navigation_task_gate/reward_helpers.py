...
class RewardHelpers:
        ...
        try:
            ...
            if fov_mag != 0.0:
                ...
                if(self.task.num_task_steps % 200 == 0):
                    ...
                    self.task.infos["static_fov/visible_fraction"] = torch.tensor(frac_visible, dtype=torch.float32)
    ...
    def post_image_reward_addition(self) -> None:
        ...
        if self.task.num_task_steps % 200 == 0:
            ...
            close_count = torch.sum((self.task.min_pixel_dist >= 2.0) & (self.task.min_pixel_dist < 4.0)).item()  # 2-4m
