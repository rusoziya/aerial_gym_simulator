    ...
    def update_gate_dimensions_for_environments(self, env_ids: torch.Tensor) -> None:
        ...
        for env_id in env_ids.tolist():
            ...
            if active_idx >= 0 and active_idx < len(env_gate_names):
                ...
                self.task.gate_height[env_id] = height
                self.task.gate_center_height[env_id] = center_height
                self.task.gate_scale_factors[env_id] = scale_factor

            else:
                # Default dimensions if no active gate found
                self.task.gate_width[env_id] = 2.5
