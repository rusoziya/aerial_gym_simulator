...
def extract_gate_dimensions_from_urdf(task, urdf_path: str) -> tuple[float, float]:
    """
...
def calculate_gate_dimensions_from_name(task, gate_name: str) -> tuple[float, float, float]:
    """
...
def update_gate_dimensions_for_environments(task, env_ids: torch.Tensor) -> None:
    """
    Update gate dimensions for specified environments based on their selected gate variants.
    """
    if not hasattr(task.sim_env, 'global_tensor_dict'):
        return
    ...
    gate_variant_names = task.sim_env.global_tensor_dict.get("gate_variant_names_per_env", [])
    active_gate_array_indices = task.sim_env.global_tensor_dict.get("active_gate_variant_array_index", torch.zeros(task.sim_env.num_envs))
    ...
    for env_id in (env_ids.tolist() if hasattr(env_ids, 'tolist') else [env_ids]):
        ...
        if active_idx >= 0 and active_idx < len(env_gate_names):
            ...
            if urdf_path and os.path.exists(urdf_path):
                width, height, center_height, scale_factor = task.extract_gate_dimensions_from_urdf(urdf_path)
            else:
                # Fallback: calculate dimensions from scale factor in filename
                width, height, center_height, scale_factor = task.calculate_gate_dimensions_from_name(active_gate_name)
            
            # Update environment-specific dimensions
            task.gate_width[env_id] = width
            task.gate_height[env_id] = height
            task.gate_center_height[env_id] = center_height
            task.gate_scale_factors[env_id] = scale_factor
            
        else:
            # Default dimensions if no active gate found
            task.gate_width[env_id] = 2.5
            task.gate_height[env_id] = 2.4
            task.gate_center_height[env_id] = 1.2
            task.gate_scale_factors[env_id] = 1.0
    ...
    if hasattr(task.sim_env, 'global_tensor_dict'):
        task.sim_env.global_tensor_dict['gate/center_height_per_env'] = task.gate_center_height.detach().clone()
