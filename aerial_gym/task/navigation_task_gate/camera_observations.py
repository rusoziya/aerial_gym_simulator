...
def process_image_observation(task) -> None:
    ...
    if (not task._drone_cam_debug_last) or (task.num_task_steps % 200 == 0):
        ...
        task._drone_cam_debug_last = task.num_task_steps
    ...
    try:
        camera_noise_disabled = bool(task.sim_env.global_tensor_dict.get('camera_randomization/noise_disabled', False))
    except (KeyError, TypeError):
        camera_noise_disabled = bool(task.disable_camera_noise_randomization)
    # Per-camera override: if set, apply to drone camera processing
    drone_noise_override = bool(task.sim_env.global_tensor_dict.get('camera_randomization/drone_noise_disabled', False)) if hasattr(task.sim_env, 'global_tensor_dict') else False
    if task.task_config.curriculum.enable_camera_noise:
        ...
        if not camera_noise_disabled and not drone_noise_override:
            ...
            gaussian_std, dropout_rate = task.task_config.curriculum.get_camera_noise(3)
    ...
    try:
        frame_dropout_disabled = bool(task.sim_env.global_tensor_dict.get('camera_randomization/frame_dropout_disabled', False))
    except (KeyError, TypeError):
        frame_dropout_disabled = bool(task.disable_camera_frame_dropout_randomization)
    drone_fd_override = bool(task.sim_env.global_tensor_dict.get('camera_randomization/drone_frame_dropout_disabled', False)) if hasattr(task.sim_env, 'global_tensor_dict') else False
    if task.task_config.curriculum.enable_camera_frame_dropout:
        ...
        if not frame_dropout_disabled and not drone_fd_override:
            ...
            fd = task.task_config.curriculum.get_camera_frame_dropout(3)
        p_blank = fd.get("drone_blank", 0.0)
        p_freeze = fd.get("drone_freeze", 0.0)
        
        # Ensure buffer exists
        if not task._prev_drone_depth is not None:
            task._prev_drone_depth = noised_image_obs.clone()
        ...
        if p_freeze > 0.0:
            ...
            noised_image_obs = torch.where(apply_freeze, task._prev_drone_depth, noised_image_obs)
    ...
    task.obs_dict["depth_range_pixels_noised"] = noised_image_obs.unsqueeze(1)  # shape: (num_envs, 1, H, W)
    
    # Encode the (potentially noisy) image using VAE
    if task.task_config.vae_config.use_vae:
        # Ensure tensor is float32, contiguous, on correct device, with channel dim
        try:
            img = noised_image_obs
            if isinstance(img, torch.Tensor):
                img = img.to(task.device, dtype=torch.float32).contiguous()
                if img.dim() == 2:
                    img = img.unsqueeze(0).unsqueeze(0).expand(task.sim_env.num_envs, -1, -1)
                elif img.dim() == 3:
                    img = img.unsqueeze(1)
            else:
                # Fallback: convert numpy to tensor
                if isinstance(img, np.ndarray):
                    img = torch.from_numpy(img).to(task.device, dtype=torch.float32).contiguous()
                    if img.ndim == 2:
                        img = img.unsqueeze(0).unsqueeze(0).expand(task.sim_env.num_envs, -1, -1)
                    elif img.ndim == 3:
                        img = img.unsqueeze(1)
            task.image_latents[:] = task.shared_vae_model.encode(img)
        except RuntimeError as e:
            logger.warning(f"VAE encoding of drone camera failed: {e}")
            task.image_latents.zero_()
        # DEBUG: Compare per-env drone VAE latents
        if (not task._drone_vae_debug_last) or (task.num_task_steps % 200 == 0):
            z = task.image_latents
            ne = int(z.shape[0])
            ...
            sames = {i: _same(i) for i in [1, 5, 8, 12]}
            task._drone_vae_debug_last = task.num_task_steps

def process_static_camera_observation(task) -> None:
    ...
    try:
        ...
        static_depth, static_seg = task.static_camera_manager.capture_images(batched=True)
        
        # CRITICAL DEBUG: Log static camera capture success/failure
        if not task._static_debug_logged:
            task._static_debug_logged = True
        ...
        if (not task._static_cam_debug_last) or (task.num_task_steps % 200 == 0):
            if hasattr(static_depth, 'shape') and getattr(static_depth, 'ndim', 0) == 3:
                ...
                task._static_cam_debug_last = task.num_task_steps
        
        if static_depth is not None and task.task_config.vae_config.use_vae:
            ...
            static_noise_override = bool(task.sim_env.global_tensor_dict.get('camera_randomization/static_noise_disabled', False)) if hasattr(task.sim_env, 'global_tensor_dict') else False
            global_noise_disabled = bool(task.sim_env.global_tensor_dict.get('camera_randomization/noise_disabled', False)) if hasattr(task.sim_env, 'global_tensor_dict') else False
            if task.task_config.curriculum.enable_camera_noise:
            ...
            # Entire-frame dropout (curriculum-driven)
            static_fd_override = bool(task.sim_env.global_tensor_dict.get('camera_randomization/static_frame_dropout_disabled', False)) if hasattr(task.sim_env, 'global_tensor_dict') else False
            global_fd_disabled = bool(task.sim_env.global_tensor_dict.get('camera_randomization/frame_dropout_disabled', False)) if hasattr(task.sim_env, 'global_tensor_dict') else False
            if task.task_config.curriculum.enable_camera_frame_dropout:
                # Decouple from noise flag: only frame-dropout flags control this schedule
                fd = task.task_config.curriculum.get_camera_frame_dropout(task.curriculum_level) if (not global_fd_disabled and not static_fd_override) else task.task_config.curriculum.get_camera_frame_dropout(3)
                p_blank = fd.get("static_blank", 0.0)
                p_freeze = fd.get("static_freeze", 0.0)
                # Initialize previous static buffer
                if not task._prev_static_depth is not None:
                    if isinstance(static_depth_noised, np.ndarray):
                        task._prev_static_depth = static_depth_noised.copy()
                    else:
                        task._prev_static_depth = static_depth_noised.clone()
                ...
                if isinstance(static_depth_noised, np.ndarray):
                    if p_blank > 0.0 and (np.random.rand() < p_blank):
                        ...
                        static_depth_noised = task._prev_static_depth.copy()
                else:
                    ...
                    if do_blank:
                        ...
                        if do_freeze:
                            static_depth_noised = task._prev_static_depth.clone()
                # update buffer
                if isinstance(static_depth_noised, np.ndarray):
                    task._prev_static_depth = static_depth_noised.copy()
                else:
            ...
            task.obs_dict["static_depth_clean"] = static_depth_clean_env0
            # If numpy/tensor batched, select env0 for GIF/debug without altering VAE input
            task.obs_dict["static_depth_noised"] = static_depth_noised[0] if (hasattr(static_depth_noised, 'ndim') and (getattr(static_depth_noised, 'ndim') == 3)) else static_depth_noised
            task.obs_dict["static_seg"] = static_seg
            
            try:
                # Convert to tensor and process through VAE (use noised version for training)
                if isinstance(static_depth_noised, np.ndarray):
                    static_depth_tensor = torch.from_numpy(static_depth_noised).float().to(task.device)
                else:
                ...
                if static_depth_tensor.dim() == 2:
                    static_depth_tensor = static_depth_tensor.unsqueeze(0).expand(task.sim_env.num_envs, -1, -1)
                elif static_depth_tensor.dim() == 3 and static_depth_tensor.shape[0] != task.sim_env.num_envs:
                    # Safe fallback: pad/trim to num_envs
                    n, h, w = static_depth_tensor.shape
                    if n < task.sim_env.num_envs:
                        reps = (task.sim_env.num_envs + n - 1) // n
                        static_depth_tensor = static_depth_tensor.repeat(reps, 1, 1)[:task.sim_env.num_envs]
                    else:
                        static_depth_tensor = static_depth_tensor[:task.sim_env.num_envs]

                # Periodic static camera depth summary (match DroneCam style)
                if (not task._static_cam_depth_logged) or (task.num_task_steps % 200 == 0):
                    task._static_cam_depth_logged = True
                ...
                if not task._vae_debug_logged:
                    task._vae_debug_logged = True
                    logger.warning(f"🔧 VAE encoding static camera: input_shape={static_depth_tensor.shape}, device={static_depth_tensor.device}")

                encoded_latents = task.shared_vae_model.encode(static_depth_tensor)
                task.static_image_latents[:] = encoded_latents

                # CRITICAL DEBUG: Verify VAE output periodically and compare across envs
                if (not task._vae_output_logged) or (task.num_task_steps % 200 == 0):
                    task._vae_output_logged = True
                    z = encoded_latents
                    ne = int(z.shape[0])
                    def _absmean_env(idx) -> None:
                        return float(torch.mean(torch.abs(z[idx])).item()) if idx < ne else float('nan')
...
def _compute_visibility_metrics(task, infos_to_return: dict[str, torch.Tensor]) -> None:
    ...
    try:
        ...
        if _flag_env or _flag_gtd or _flag_dbg:
            ...
            have_cam = (scm is not None and hasattr(scm, 'last_camera_pos') and hasattr(scm, 'last_camera_target')
                        and len(scm.last_camera_pos) >= task.num_envs and len(scm.last_camera_target) >= task.num_envs)
            if have_cam:
                cam_pos = torch.tensor(scm.last_camera_pos, dtype=torch.float32, device=task.device)
                cam_tgt = torch.tensor(scm.last_camera_target, dtype=torch.float32, device=task.device)
            else:
                cam_pos = None
            # Fallback: synthesize camera pose from base_y/base_z and gate center if caches missing
            if cam_pos is not None:
                # Camera basis (right, up, forward)
                up_world = torch.tensor([0.0, 0.0, 1.0], dtype=torch.float32, device=task.device).view(1, 3).expand(task.num_envs, 3)
                fwd = cam_tgt - cam_pos
                ...
                W = task.gate_width.view(task.num_envs, 1, 1)
                H = task.gate_height.view(task.num_envs, 1, 1)
                gx = task.gate_position[:, 0].view(task.num_envs, 1, 1)
                gy = task.gate_position[:, 1].view(task.num_envs, 1, 1)
                gz = task.gate_position[:, 2].view(task.num_envs, 1, 1)
                # Grid coordinates (center-sampled)
                xi = (torch.arange(N, device=task.device, dtype=torch.float32) + 0.5) / float(N) - 0.5  # [-0.5, 0.5]
                zj = (torch.arange(M, device=task.device, dtype=torch.float32) + 0.5) / float(M)         # [0, 1]
                # Broadcast-friendly grid centers
                X = gx + (W * xi.view(1, N, 1))            # (E,N,1)
                Z = gz + (H * zj.view(1, 1, M))            # (E,1,M)
                Y = gy.view(task.num_envs, 1, 1)           # (E,1,1)
                # Expand to (E,N,M)
                X = X.expand(task.num_envs, N, M)
                Z = Z.expand(task.num_envs, N, M)
                Y = Y.expand(task.num_envs, N, M)
                # Assemble world points (E,N,M,3)
                cam_pos_ = cam_pos.view(task.num_envs, 1, 1, 3)
                Pw = torch.stack([X, Y, Z], dim=3) - cam_pos_
                # Project to camera basis
                rx = right.view(task.num_envs, 1, 1, 3)
                upv = up.view(task.num_envs, 1, 1, 3)
                fv = fwd.view(task.num_envs, 1, 1, 3)
                ...
                drone_pos = task.obs_dict.get('robot_position', None)
                if isinstance(drone_pos, torch.Tensor) and drone_pos.shape[0] == task.num_envs:
                    cpos = cam_pos_
                    qpts = torch.stack([X, Y, Z], dim=3)
                    seg = qpts - cpos
                    vv = torch.sum(seg * seg, dim=3)  # |v|^2
                    w = drone_pos.view(task.num_envs, 1, 1, 3) - cpos
                    t = torch.sum(w * seg, dim=3) / torch.clamp(vv, min=1e-6)
                    t = torch.clamp(t, 0.0, 1.0).unsqueeze(3)
                    p_closest = cpos + t * seg
                    d2 = torch.sum((p_closest - drone_pos.view(task.num_envs, 1, 1, 3)) ** 2, dim=3)
                    try:
                ...
                task.infos["static_visibility/abs"] = vis_frac.detach()
                task.infos["static_visibility/frustum"] = frustum_frac.detach()
                task.infos["static_visibility/eff"] = eff.detach()
                ...
                try:
                    ...
                    if debug_on and task.num_envs > 0:
                        ...
                        try:
                            ...
                            ep_idx0 = max(0, len(task.completed_episodes) - 1)
                        cx, cy, cz = float(cam_pos[e0, 0].item()), float(cam_pos[e0, 1].item()), float(cam_pos[e0, 2].item())
                        gx0 = float(task.gate_position[e0, 0].item()); gy0 = float(task.gate_position[e0, 1].item()); gz0 = float(task.gate_position[e0, 2].item())
                        w0 = float(task.gate_width[e0].item()); h0 = float(task.gate_height[e0].item())
    ...
    try:
        ...
        if have_cam:
            ...
            robot_position = task.obs_dict.get('robot_position', torch.zeros((task.num_envs, 3), device=task.device))
            ...
            task.infos["static_fov/visible"] = visible.float()
            task.infos["static_fov/horiz_angle_rad"] = horiz_angle
            task.infos["static_fov/vert_angle_rad"] = vert_angle
            task.infos["static_fov/score"] = fov_score
            ...
            # Optional env0 debug
            if _os.environ.get('VISIBILITY_DEBUG', '').strip().lower() in ('1', 'true', 'yes', 'y') and task.num_envs > 0:
                e0 = 0
                step_ep0 = int(task.episode_lengths[0].item())
    except (ValueError, TypeError):
        pass

def _get_static_camera_pose_relative_to_drone(task) -> torch.Tensor:
    ...
    try:
        base_y = float(task.task_config.static_camera_base_y)
    except (ValueError, TypeError):
        base_y = -3.0
    ...
    adaptive_z = False
    base_z_value = 1.5
    cfg_base_z = task.task_config.static_camera_base_z
    ...
    dynamic_enabled = bool(task.task_config.curriculum.enable_dynamic_camera_following)
    try:
        dyn_dis = bool(task.sim_env.global_tensor_dict.get('dynamic_camera_following/disabled', False)) if hasattr(task.sim_env, 'global_tensor_dict') else False
    except (KeyError, TypeError):
        dyn_dis = False
    ...
    if dynamic_effective:
        ...
        try:
            robot_pos_world = task.obs_dict['robot_position'].to(device=device, dtype=torch.float32)
        except (KeyError, TypeError):
    ...
    try:
        robot_pos = task.obs_dict['robot_position'].to(device=device, dtype=torch.float32)
    except (KeyError, TypeError):
    ...
    q = None
    try:
        q = task.obs_dict.get('robot_vehicle_orientation', None)
    except (KeyError, TypeError):
        q = None
    if q is None:
        try:
            q = task.obs_dict.get('robot_orientation', None)
        except (KeyError, TypeError):
            q = None
    if q is None:
    ...
    gate_pos_world = task.gate_position
    gcz = task.gate_center_height
    ...
    task._debug_cam_world = cam_world.detach().clone()
    task._debug_rel_pos = rel_pos_body.detach().clone()
    task._debug_rel_eul = rel_orient_euler.detach().clone()
    task._debug_cam_eul = eul_cam.detach().clone()
    task._debug_drone_eul = drone_eul_world.detach().clone()
