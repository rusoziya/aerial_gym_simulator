...
def _validate_and_step(task, actions: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    """Transform actions, run physics step, detect NaN/Inf in actions and observations."""
    transformed_action = task.action_transformation_function(actions)
    ...
    try:
        ...
        if torch.any(invalid_action_mask):
            ...
            nan_trunc_mask = torch.zeros(task.num_envs, dtype=torch.bool, device=task.device)
    except RuntimeError:
        nan_trunc_mask = torch.zeros(task.num_envs, dtype=torch.bool, device=task.device)

    task.sim_env.step(actions=transformed_action)

    # Observation NaN/Inf guard
    for k, v in task.obs_dict.items():
        if isinstance(v, torch.Tensor) and v.shape[0] == task.num_envs:
            bad = torch.any(torch.isnan(v) | torch.isinf(v), dim=tuple(range(1, v.ndim)))
            if task.task_config.guard_debug_enabled and torch.any(bad):
    ...
    if torch.any(nan_trunc_mask):
        if "robot_position" in task.obs_dict and isinstance(task.obs_dict["robot_position"], torch.Tensor):
            task.obs_dict["robot_position"][nan_trunc_mask] = 0.0
...
def _detect_gate_passage(
    ...
    gate_success_width_tolerance = task.gate_width * 0.50
    gate_success_min_height = task.gate_position[:, 2]  # gate bottom
    gate_success_max_height = task.gate_position[:, 2] + task.gate_height  # gate top

    gate_passage_success = (
        (robot_position[:, 1] > task.gate_position[:, 1])
        & (torch.abs(robot_position[:, 0] - task.gate_position[:, 0]) < gate_success_width_tolerance)
    ...
    x_off_imm = torch.abs(robot_position[:, 0] - task.gate_position[:, 0])
    z_off_imm = torch.abs(robot_position[:, 2] - (task.gate_position[:, 2] + task.gate_center_height))
    x_ok_imm = x_off_imm <= (task.gate_width * 0.10)
    z_ok_imm = z_off_imm <= (task.gate_height * 0.10)
    ...
    immediate_success_mask = (~(task.terminations > 0)) & gate_passage_success
    if torch.any(immediate_success_mask):
        ...
        task.terminations[immediate_success_mask] = 1
        # Record per-episode target success flags where 10% tolerance is also met
        task._ep_target_success_flag[immediate_success_mask] |= target_success_immediate[immediate_success_mask]
        try:
    ...
    crash_mask = (task.obs_dict["crashes"] > 0)
    successes = (task.terminations > 0) & gate_passage_success & (~crash_mask)

    # Target success at truncation: same 10% width/height window around adaptive gate center
    x_off = torch.abs(robot_position[:, 0] - task.gate_position[:, 0])
    z_off = torch.abs(robot_position[:, 2] - (task.gate_position[:, 2] + task.gate_center_height))
    x_ok = x_off <= (task.gate_width * 0.10)
    z_ok = z_off <= (task.gate_height * 0.10)
    target_success = x_ok & z_ok
    target_successes = (task.terminations > 0) & (target_success & gate_passage_success) & (~crash_mask)
    # Also accumulate per-episode target success flag when truncated at step end
    end_success_mask = (task.terminations > 0) & (target_success & gate_passage_success) & (~crash_mask)
    task._ep_target_success_flag[end_success_mask] = True
...
def _apply_timeout_and_populate_infos(
    ...
    """Compute timeout flags, populate task.infos, and apply timeout penalty to rewards."""
    timeouts = torch.where(
        task.truncations > 0, torch.logical_not(successes), torch.zeros_like(successes)
    )
    timeouts = torch.where(
        task.terminations > 0, torch.zeros_like(timeouts), timeouts
    )  # timeouts are not counted if there is a crash

    task.infos["successes"] = successes
    task.infos["timeouts"] = timeouts
    # Report crashes only (exclude success-based terminations)
    task.infos["crashes"] = task.obs_dict["crashes"]

    # One-off timeout penalty: discourage hover-to-horizon strategies
    try:
        timeout_penalty = float(task.task_config.reward_parameters.get('timeout_penalty', 70.0))
    except (ValueError, TypeError):
        timeout_penalty = 75.0
    if torch.any(timeouts):
        # Apply to the per-env reward vector maintained at the task level
        task.rewards = task.rewards - (timeouts.float() * timeout_penalty)
        task.episode_timeout_penalty[timeouts] -= timeout_penalty
...
def _compute_gate_navigation_metrics(
    ...
    """Calculate gate navigation metrics and populate task.infos with tracking data.
    ...
    gate_tracking_width_tolerance = task.gate_width * 0.6  # 60% of gate width for tracking
    gate_tracking_min_height = task.gate_position[:, 2] + task.gate_height * 0.08  # 8% above ground
    gate_tracking_max_height = task.gate_position[:, 2] + task.gate_height * 0.92  # 92% of gate height

    gate_passed_current = (
        (robot_position[:, 1] > task.gate_position[:, 1])
        & (torch.abs(robot_position[:, 0] - task.gate_position[:, 0]) < gate_tracking_width_tolerance)
    ...
    gate_alignment = torch.abs(robot_position[:, 0] - task.gate_position[:, 0]) < gate_tracking_width_tolerance
    ...
    task.infos["gate/passed"] = gate_passed_current.float()
    task.infos["gate/distance"] = gate_distance
    task.infos["gate/alignment"] = gate_alignment.float()
    task.infos["camera/facing_alignment"] = camera_gate_alignment
    task.infos["camera/alignment_angle_deg"] = alignment_angle_deg
    task.infos["camera/alignment_category"] = alignment_category
...
def _update_trajectory_state(
    ...
    if torch.any(fresh_mask):
        task._ep_spawn_pos[fresh_mask] = robot_position[fresh_mask]
        _gcenter = task.gate_position.clone()
        _gcenter[:, 2] = _gcenter[:, 2] + task.gate_center_height
        task._ep_gate_center_at_spawn[fresh_mask] = _gcenter[fresh_mask]
        task._ep_last_pos[fresh_mask] = robot_position[fresh_mask]
        task._episode_fresh[fresh_mask] = False

    step_deltas = robot_position - task._ep_last_pos
    step_dist = torch.norm(step_deltas, dim=1)
    task._ep_path_len += step_dist
    task._ep_last_pos = robot_position
    task._ep_steps += 1

    step_gate_dist = torch.norm(robot_position - gate_center_position, dim=1)
    task._ep_min_gate_dist = torch.minimum(task._ep_min_gate_dist, step_gate_dist)

    newly_crossed = (~task._ep_gate_crossed) & gate_passed_current
    if torch.any(newly_crossed):
        task._ep_gate_crossed[newly_crossed] = True
        task._ep_time_to_gate[newly_crossed] = task._ep_steps[newly_crossed].to(torch.float32)
        ...
        task._ep_center_offset_cross[newly_crossed] = torch.sqrt(dx_cross * dx_cross + dz_cross * dz_cross)
        task._ep_height_offset_cross[newly_crossed] = torch.abs(dz_cross)
...
def _handle_post_reward_reset(
    ...
    try:
        env_ids = reset_envs if torch.is_tensor(reset_envs) else torch.tensor(reset_envs, device=task.device, dtype=torch.long)
        # Path efficiency = path length / straight-line distance from spawn to gate center at spawn
        denom = torch.norm(task._ep_spawn_pos[env_ids] - task._ep_gate_center_at_spawn[env_ids], dim=1)
        denom = torch.clamp(denom, min=1e-6)
        # Fallback for rare cases where incremental path stayed ~0 (e.g., immediate reset)
        disp = torch.norm((robot_position[env_ids] - task._ep_spawn_pos[env_ids]), dim=1)
        path_len = task._ep_path_len[env_ids]
        path_len = torch.where(path_len <= 1e-6, disp, path_len)
        path_eff = torch.full((task.num_envs,), float('nan'), device=task.device)
        path_eff[env_ids] = (path_len / denom).clamp(max=1000.0)
        # Time to gate in steps (already NaN for non-crossers)
        time_to_gate = task._ep_time_to_gate.clone()
        # Min distance to gate center during episode
        min_gate_dist = task._ep_min_gate_dist.clone()
        # Offsets at crossing (NaN for non-crossers)
        center_offset = task._ep_center_offset_cross.clone()
        height_offset = task._ep_height_offset_cross.clone()
        ...
        try:
            ...
            overall_success_rate = torch.tensor(float('nan'), device=task.device)
            target_success_rate = torch.tensor(float('nan'), device=task.device)
        # Stash per-env episode metrics for worker-side running aggregation
        task._stash_per_env_trajectory_metrics(
        ...
        task._stash_averaged_trajectory_metrics(
        ...
        task._populate_episode_extra_stats()
...
def _populate_episode_extra_stats(task) -> None:
...
def _process_images_and_finalize(task) -> None:
    """Run image processing, static camera updates, and one-shot final verification."""
    task.process_image_observation()
    task.process_static_camera_observation()
    task.post_image_reward_addition()

    # FINAL VERIFICATION: After all processing is complete
    if not task._final_verification_printed:
        task._final_verification_printed = True
        ...
        if 'observations' in task.task_obs:
            obs_sample = task.task_obs["observations"][0]
